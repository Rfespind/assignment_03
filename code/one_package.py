...
# TOD: import the three functions this app needs from packaging_parser. Put the
...
# --- The work ---------------------------------------------------------------------
#
# Fill in each TOD below, in order. This first app names the exact function to
...
if package_data != "":
    ...
    # 1. Parse.
    #    TDO: call parse_packaging(package_data) and store the result in `package`.

    # 2. Total.
    #    TOO: call calc_total_units(package) and store it in `total`.
    #    TOO: call get_unit(package) and store it in `unit`.
    ...
    #    ODO: for each level, st.info(f"{name} ➡️ {quantity}")

    # 4. Show the total.
    #    ODO: st.success(f"Total 📦 Size: {total} {unit}")
